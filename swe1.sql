...
--
-- Host: 127.0.0.1:3306
-- Generation Time: May 04, 2018 at 02:04 PM
...
INSERT INTO `child` (`id`, `ddoe`, `main_id`) VALUES
(1, 4, 2),
(2, 5, 4),
(3, 5, 4);
...
CREATE TABLE IF NOT EXISTS `package` (
  ...
  `discount` int(11) NOT NULL,
  `description` varchar(500) NOT NULL,
  `parent_id` int(11) NOT NULL COMMENT 'since the parent is the one that pays we can assume that, parent table is the one with the right to get a package reducingthe cost paid and thus extending the same thing to their children, so parent_id is better than main_id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `salary` (
  ...
  PRIMARY KEY (`id`),
  KEY `bonuses_id` (`bonuses_id`),
  KEY `deductables_id` (`deduct_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
ALTER TABLE `salary`
  ADD CONSTRAINT `salary_ibfk_2` FOREIGN KEY (`bonuses_id`) REFERENCES `bonus` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `salary_ibfk_3` FOREIGN KEY (`deduct_id`) REFERENCES `deduct` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
